...
fn main() {
    ...
    let first_Smart_pointer = CustomSmartPointer {
        data: String::from("first smartpointer"),
    };

    // lets say for some reason we wanted to delete 'first_Smart_pointer' then we need to use  std::mem::drop
    drop(first_Smart_pointer); // it will dropped first and then

    let second_Smart_pointer = CustomSmartPointer {
        data: String::from("second smartpointer"),
    };
